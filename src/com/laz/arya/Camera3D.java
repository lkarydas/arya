...
import com.laz.utils.Quaternion;
...
public class Camera3D {
	...
	public void setYawAndPitch(float yaw, float pitch)
	{
		...
		currentYaw += yaw/10;
		//currentPitch += pitch/10;
		
		// Set yaw and pitch
		//rotate(up, currentYaw);
		
		
		//rotate(RIGHT, currentPitch);
		position.x += yaw/10;   // Debugging the camera projection distorition
		
		Log.i("3D_stuff", "Position: " + position.toString());
		Log.i("3D_stuff", "Direction: " + direction.toString());
		Log.i("3D_stuff", "Up: " + up.toString());
		update();
		
		view.print("Camera View Matrix");
	...
	public void update(){
		...
		projection.setToProjection(Math.abs(near), Math.abs(far), 45, aspect);
		// Set view matrix
