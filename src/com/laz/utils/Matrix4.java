...
public class Matrix4 implements Serializable {
	...
	public Matrix4 mul (Matrix4 matrix) {
		//TODO: This is done to bypass the libgdx native code for multiplication but it's possibly slow
		float[] result = new float[16];
		Matrix.multiplyMM(result, 0, this.val, 0, matrix.val, 0);
		this.set(result);
		return this;
	}
	...
	public void print(String title){
		...
		for (int i = 0; i < 4; i++){
			line = "";
			for (int j = 0; j < 4; j++){
				line += String.format(Locale.US,"%.2f ", this.val[4*j + i]);
			}
			Log.i("3D_stuff", line);
		}
	}
